<!-- Footer -->
<footer>
  <div class="container text-center text-md-start">
    <div class="row">
      <!-- Logo & Description -->
      <div class="col-md-4 mb-4 mb-md-0 d-flex flex-column align-items-center align-items-md-start">
        <div class="d-flex align-items-center mb-2">
          <img src="../assets/images/logos/logo.svg" alt="Bong Bicycle Shop Logo" style="width:40px; height:40px; object-fit:contain; margin-right:10px;">
          <span class="navbar-brand-text" style="font-size:1.3rem; font-weight:700;">Bong Bicycle Shop</span>
        </div>
        <p style="max-width:260px;">Your one-stop shop for bikes, parts, and cycling guides at the best prices.</p>
      </div>
      <!-- Quick Links -->
      <div class="col-md-4 mb-4 mb-md-0">
        <h5><strong>Quick Links</strong></h5>
        <ul class="list-unstyled">
          <li>
            <a class="footer-link d-flex align-items-center" href="../front-pages/index.php">
              Home
            </a>
          </li>
          <li>
            <a class="footer-link d-flex align-items-center" href="../front-pages/about.php">
              About Us
            </a>
          </li>
          <li>
            <a class="footer-link d-flex align-items-center" href="../front-pages/contact.php">
              Contact Us
            </a>
          </li>
          <li>
            <a class="footer-link d-flex align-items-center" href="../front-pages/guides.php">
              Guides
            </a>
          </li>
        </ul>
      </div>
      <!-- Shop Information -->
      <div class="col-md-4">
        <h5><strong>Shop Information</strong></h5>
        <div class="mb-2">
          <i class="bi bi-envelope me-2"></i> bongbicycleshop@gmail.com
        </div>
        <div class="mb-2">
          <i class="bi bi-geo-alt me-2"></i> 688 Congressional Rd, General Mariano Alvarez, Cavite
        </div>
        <div class="mt-3">
          <strong>Service Hours</strong><br>
          <i class="bi bi-clock me-2"></i> Mon - Sat: 8:00 AM - 5:00 PM
        </div>
      </div>
    </div>
    <hr class="border-top" />
    <p class="mt-3 mb-0 text-center">&copy; 2025 Bong Bicycle Shop. All rights reserved.</p>
  </div>
</footer>